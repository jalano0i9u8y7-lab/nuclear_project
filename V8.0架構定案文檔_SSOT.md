# Nuclear Project SSOT V8.0 架構定案文檔
...
- **封存日期**: 2026-01-17
- **系統設計完成日期**: 2026-01-19
- **最後更新**: 2026-02-03（⭐ **V8.43**：D-1B 論壇白名單 V1 與搜尋策略；D-1A 與 D-1B 全程分離原則—分類/索引/D3/D4 須分欄註明權威新聞 vs 散戶論壇，不得互相汙染或混淆）（⭐ **V8.42**：檢查報告採納補強—C4 Python 強制 include_reasoning/reasoning_trace；C5 P2_1_FACT_MODEL 最小介面；C6 escalation_path 狀態標記；S2 D-3 與 P6 數據流釐清；S3 防重複強制；S4 板塊定義一致性；S5 關鍵資產 V1 與上限；O1–O3 不進 SSOT、見工程備忘錄）（⭐ **V8.41**：平台說明 GAS 已中止、全面搬遷 Python；設計名詞對照 D-1～D-4／WB-1／WB-2／W-A 為目標架構，P5-B／P5-A 僅 GAS 對照；SSOT 更新勿用「最新」、僅標版本流水號與最後更新日期；GAS 測試流程與規則 2 已廢止；定案補充 1–10 已納入）（⭐ **V8.40 P2.5 與 Daily/Weekly 定案**：P2.5 大框架月/季度級機構數據蒐集與分析、作為每月/季全部個股重跑世界觀的程序之一、月/季重跑時若機構籌碼面數據已是 P5 每日蒐集則直接引用 P5 不需重蒐集；高頻蒐集低頻引用不重複工作；D-3 個股專屬全方位資訊紀錄統整師、產出當天最新+歷史前三天關聯快照與狀態報告、可標註變化/異常/延續/轉折但不分析漲跌或策略；D-4 納入討論串所有衍生性與宏觀指標、個股與宏觀 10–12 大類分清楚；所有快照交給決策系統與動態學習系統；三層架構為「工作搭配正確處理端」原則非 phase 對應；Daily 最高目標為時間連續世界模型、不寫死「不分析」、須連接歷史與統整摘要；程式數據來源與公式摘要須記錄；Python 有更好數據來源就取代 GAS 妥協、不帶舊妥協到新系統）（⭐ **V8.39 Daily 蒐集清單與差異清單**：`Daily每日蒐集清單與白名單_SSOT級_2026-01-26.md` 列出 Daily 每天需蒐集的所有數據、來源必須白名單、Python 數據來源選項；`P2.5與Daily_Weekly修改差異清單_待最終確認.md` **已定案（V8.40 用戶確認）**，定案補充 1–10 已納入本 SSOT；工程備忘錄新增「Python 開發標準流程」、`SSOT細節缺口與遷移備忘錄` 正式寫程式時須再對照舊碼）（⭐ **V8.38 Prompt 對齊與 SSOT 缺口**：`Prompt對齊與銜接原則_2026-01-26.md` 規定所有 Prompt 承襲 GAS、對齊憲法與架構、不遺漏既有功能、前後銜接與最高目標/產出精準；`SSOT細節缺口檢查報告_2026-01-26.md` 標出非 Prompt 之細節缺口並建議 SSOT 補強；`P2.5與Daily_Weekly微調討論整理_待定案.md` 為 P2.5 月度機構級、D3/D4 角色與宏觀分類 V2.0、三層架構等討論整理，**待用戶定案後再更新 SSOT**）（⭐ **V8.37 遷移與衍生品報告**：`Prompt遷移確認報告_GAS至Python_2026-01-26.md` 列出 GAS 全部 build*Prompt/build*Auditor 函數與遷移檢查清單，確保改寫 Python 時 Prompt 無遺漏；`衍生品與數據收集監控分析完整報告_2026-01-26.md` 整理衍生品/數據收集/監控/分析完整內容與步驟對照）（⭐ **V8.36 架構提前納入**：Hermes 4 輸出表須含 reasoning_trace/final_answer；Order DSL 須帶 worldview_version/identity_context；P6 清除鎖與保留 flag；DSFP worldview 快照分支；見資料存儲專節）（⭐ **Hermes 4 思考過程必存**：思考過程全部計費，須強制 `include_reasoning` 並將 `<think>` 內容存入 `reasoning_trace`，見架構總覽 M0 與工程備忘錄第四節）（⭐ **Hermes 4 必開 Think Mode**：見架構總覽 M0）（⭐ V8.35 審查回覆定案：P2 總述與下游讀 Phase2_Output、**附錄：GAS 時代／現行設計對照**（方案 A，舊設計集中、主文不混雜）、Daily/世界觀重建澄清、**歷史與現狀銜接原則**（前三日快照＋同一觀察者）、P6 清除改為本週策略產出後清除上週、保留兩週、僅 P6 數據不含共用 OHLCV、D-4 各宏觀商品現狀快照與 DSFP 宏觀數據由 Daily、Batch fallback、記憶 DB 歸檔、P3 週線可來自數據源、Learning 防重複、P0.5 Mode 2 條件、**殘影→20 分鐘動能追蹤**統一命名）（⭐ V8.34 架構師級審查）✅ **系統設計階段完成，進入測試流程**
...
- **⭐ V8.17 持股財報完整分析系統**：P4 完成後自動生成（一次完成：五年歷史經驗 + 預估今年財報日期），使用 Sonnet 4.5（正式模式）或 Gemini 2.5 Lite（測試模式），Daily 新聞更新使用 AI 分類（`Earnings_Date_Announcement`）✅ **已完成**
...
- **配置內容**（⭐ V8.29 更新：Scout 模型與無 Scout/Deep 之 Phase 明列）：
  - **P0**: Claude Opus 4.5（執行者）+ Scout GPT-5.2 → 分歧時 Deep Auditor o3
  - **P0.5**: Claude Opus 4.5（執行者）+ Scout GPT-5.2 → 分歧時 Deep Auditor o3
  - **P0.7**: O3（執行者）+ Claude Opus 4.5（審查者）；**無 Scout/Deep Auditor**
  ...
  - **P2_5**: Claude Sonnet 4.5（執行者）+ Scout GPT-5.2 → 分歧時 Deep Auditor o3
  - **P3**: Claude Sonnet 4.5（執行者）+ Scout GPT-5.2（觸發式審查 15-25%）→ 分歧時 Deep Auditor o3
  - **P5_DAILY**：拆為 D-1～D-4 四模型（見下「P5 Daily 工作拆分」）；**D-1～D-4 皆無 Scout/Deep Auditor**
  - **P5_WEEKLY**:
    - **P5-B**: Claude Sonnet 4.5（執行者）+ 無審查者（只用 Validator）
    - **P5-A**: Claude Opus 4.5（執行者）+ Scout GPT-5.2（10-20%）→ 分歧時 Deep Auditor o3
  - **P5_MONTHLY / P5_QUARTERLY**: Claude Sonnet 4.5（執行者）+ Scout GPT-5.2 → 分歧時 Deep Auditor o3
...
- **AI 模型**: O3（執行者）+ Claude Opus 4.5（審查者）
- **⭐ 重要**: 純學術硬底子分析，**不包含任何籌碼面分析**
...
- **AI 模型**: Claude Sonnet 4.5（執行者）+ GPT-5.2（審查者）
...
- **AI 模型**: Claude Sonnet 4.5（執行者）+ GPT-5.2（審查者）⭐ V8.16 補強配置
- **⭐ V8.16 模型配置補強**：
  - **配置位置**：`src/02_M0_CONFIG.js` 的 `TASK_TO_EXECUTOR` 和 `getAuditor()`
  - **P2_5_MONTHLY**: Claude Sonnet 4.5（執行者）+ GPT-5.2（審查者）
  - **P2_5_QUARTERLY**: Claude Sonnet 4.5（執行者）+ GPT-5.2（審查者）
- **⭐ 對沖基金 Clone 詳細實現（需補強）**：
...
- **⭐ 數據來源（白名單原則）**：
  ...
  - **所有數據都由程式從白名單數據源獲取，不讓 AI 模型自己去找**
- **AI 模型**: Claude Sonnet 4.5（執行者，**Batch API**）+ GPT-5.2（審查者，**觸發式審查**，只審 15-25%）⭐ V8.16 更新 + ⭐ V8.17 更新（Batch API）
- **⭐ V8.17 Batch API 整合**：
...
- **⭐ V8.17 新增：P4 完成後自動觸發持股財報完整分析**：
  ...
  - **AI 模型**：Sonnet 4.5（正式模式，Batch API）或 Gemini 2.5 Lite（測試模式，同步 API）
...
- **⭐ 數據來源（白名單原則 + GOOGLEFINANCE）**：
  - **OHLCV 數據**：優先使用 GOOGLEFINANCE（免費、穩定），備用 stooq.com（通過 Cloud Function 代理）、TWSE/TPEX（台股官方數據源）
...
##### **P5.4: 監測持股大幅度波動或突發黑天鵝新聞** ⭐ V8.0 調整（已搬移到 P6）
...
##### **P5.5: 財報事件監控與策略執行** ⭐ V8.0 重構（職責分工 + 盤中監控搬移到 P6，Daily 不再監控）
- **檔案**: 
...
##### **P5.6: 按照台股當天 OHLCV 檢查是否有到目前設定的買入/賣出價位並提醒使用者** ⭐ V8.0 調整（已搬移到 P6）
...
5. **持股財報完整分析系統**：100% 完成 ✅
   ...
   - AI 模型配置：Sonnet 4.5（正式模式，Batch API）或 Gemini 2.5 Lite（測試模式，同步 API）✅
...
### 附錄 A：Weekly 雙層 AI 架構（P5-B / P5-A）— GAS 現行

- **⭐ V8.15 待實現：雙層 AI 架構（P5-B / P5-A）** ✅ **已實現** + **⭐ V8.16 更新（P5-B 移除審查者，改用 Validator）**
  - **Layer 1：P5-B（Weekly State Evaluator）**：每檔都跑（低成本，Claude Sonnet 4.5）+ **無審查者**（只用 Validator）⭐ V8.16 更新
  ...
  - **Layer 2：P5-A（Weekly Deep Re-evaluation）**：僅升級少數（10-20%），Claude Opus 4.5（執行者）+ GPT-5.2（審查者）；觸發 Gate：`escalation_score > 0.6`、`trend_integrity < 0.4`、`distribution_risk > 0.7`、`chain_dynamics_signal === "DIVERGENCE"`、`p2_5.insider_selling_alert === true`、`p2_5.abnormal_13f_distribution === true`
  - **✅ 已實現**：`P5_B_Execute()`、`P5_A_Execute()`、P5-B Validator、P5-B/P5-A Batch API、P5 Weekly Core 整合 Batch 結果處理（GAS 實現位置見主文相關章節或程式碼）。

---

**報告完成日期**：2026-01-26  
**狀態**：✅ **增強 1-4 已完成**（GAS 時代已實現；對照 gas_archive），⚠️ **增強 5 待長期實施**；✅ **V8.28 SSOT 補齊與全系統 AI 工作紀錄已完成**；✅ **附錄：GAS 時代／現行設計對照**（方案 A，舊設計集中、主文不混雜）
